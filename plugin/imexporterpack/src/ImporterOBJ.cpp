...
ImporterOBJ::ImporterOBJ(QWidget* parent)
 : Importer(parent)
{
	//Initialize the UI
	m_ui.setupUi(this);
	...
	connect(m_ui.selectMaterialDirectoryButton, SIGNAL(clicked()), this , SLOT(selectMaterialDirectory()));
	connect(m_ui.OKButton, SIGNAL(clicked()), this , SLOT(execute()));	
}

void ImporterOBJ::selectFileName()
{
...
void ImporterOBJ::selectTextureDirectory()
{
...
void ImporterOBJ::selectMaterialDirectory()
{
 	QString sDir = QFileDialog::getExistingDirectory(NULL, 
 		"Open material directory", m_ui.textureDirectoryLineEdit->text(),
		QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);	
	...
	m_ui.materialDirectoryLineEdit->setText(sDir);	
}

bool ImporterOBJ::execute()
{	
	setVisible(false);          	
	...
	QString sMatDir = m_ui.materialDirectoryLineEdit->text();
	if(sMatDir=="")
	{
		int nIndex = max(sFileName.lastIndexOf("/"), sFileName.lastIndexOf("\\"));
		if(nIndex != -1)
			sMatDir = sFileName.left(nIndex);
	}

	ImporterOBJLoader loader(sMatDir.toStdString());
	...
	//Create data from the OBJ-rawdata
	// <=, because the last one should create the mesh with the default-material
...
void ImporterOBJ::setMaxSizeAndCenter(vector< vector<Vertex> >& verts, double dSize,
	bool dCenter) const
{
