/***************************************************************************
 *   Copyright (C) 2006-07 by Reinhard Jeschull/Philipp Gruber
...
/*! \file
* 
*  \brief Describes the class BlockGen, their methodes and attributes.
*  \author Philipp Gruber
* 
*/


/*! \brief The Block generator
*
* This plugin class generates a Block and adds it to the scene
*/
class BlockGen : public BlockGenInterface
{
	Q_OBJECT
...
public: 
	...
	QString getAutor()
	{
		return "Reinhard Jeschull/Philipp Gruber";
	}
...
private:
	/*!< \brief all Sides of the Block*/
	enum Sides {Top, Bottom, Front, Back, Left, Right};
	
	/*! \brief Creates a Side of the Block
	* 
	...
	* \param dWidth The width of the side
	* \param dHeight The height of the side
	* \param dDepth The depth of the Block
	* \param nSegmentsWidth Number of segments on X-Axis
	* \param nSegmentsHeight Number of segments on Y-Axis
	* \param side The current side
	* \param nCurIndex The current Index
	* \param nCurVertex The current Vertex
	*/	
	void createSide(vector<int>& indices, vector<Vertex>& verts,
		double dWidth, double dHeight, double dDepth, int nSegmentsWidth, 
		int nSegmentsHeight, Sides side, UINT& nCurIndex, UINT& nCurVertex);
		
private: 
	Version version;					/*!< \brief The version of the plugin*/
	QPushButton* createBlockButton;		/*!< \brief The button for the toolbox*/
